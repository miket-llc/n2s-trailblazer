...
echo "🚀 EMBEDDING ALL RUNS - 163,444+ DOCUMENTS (FIXED TOKEN LIMITS)"
echo "=================================================================="
...
for run_id in "${run_ids[@]}"; do
    ...
    # Run with text-embedding-3-large (higher context limit) and smaller batches
    if trailblazer embed load --run-id "$run_id" --provider openai --model text-embedding-3-large --batch 32 --reembed-all; then
        echo "✅ Success: $run_id"
    else
