...
def test_bootstrap_doc_missing_enriched(mock_session_factory, mock_embedder):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        chunks_file = create_run_without_enriched(temp_path, run_id)
        ...
        with (
            ...
            doc_call_args = mock_upsert_doc.call_args[0][
                1
            ]  # Second arg is doc_data
            ...
            assert (
                doc_call_args["url"]
                == "https://confluence.example.com/display/SPACE/Test+Document"
            )
            ...
            expected_hash = hashlib.sha256(
                "This is the content of the document chunk".encode("utf-8")
            ).hexdigest()
...
def test_bootstrap_doc_mismatched_enriched(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        chunks_file, enriched_file = create_run_with_mismatched_enriched(
            temp_path, run_id
        )
...
def test_bootstrap_doc_minimal_traceability(
    mock_session_factory, mock_embedder
):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with (
            ...
            assert (
                doc_call_args["title"] == "Document minimal_doc"
            )  # Default title
            assert (
                doc_call_args["source_system"] == "unknown"
            )  # Default source
...
def test_bootstrap_doc_existing_document_updates_hash(
    mock_session_factory, mock_embedder
):
...
def test_bootstrap_doc_no_traceability_chunk_id_fallback(
    mock_session_factory, mock_embedder
):
