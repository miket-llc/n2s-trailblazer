...
class TestJSONLSchema:
    ...
    def test_embed_logging_schema(self):
        ...
        with (
            ...
            patch("trailblazer.pipeline.steps.embed.loader.get_session_factory") as mock_session_factory,
            ...
            if output.strip():
                ...
                for event in events:
                    ...
                    if event.get("stage") == "embed":
                        assert "provider" in event, "Embed event missing provider"
                        assert "dimension" in event, "Embed event missing dimension"

                    # Check timestamp format (ISO 8601)
                    ts = event["ts"]
                    assert "T" in ts and ("Z" in ts or "+" in ts), f"Invalid timestamp format: {ts}"
    ...
    def test_chunk_logging_schema(self):
        ...
        with patch("trailblazer.pipeline.steps.chunk.engine.emit_event") as mock_emit:
            ...
            for call in mock_emit.call_args_list:
                ...
                if args:
                    ...
                    assert "chunk." in event_type, f"Event type should be namespaced: {event_type}"
    ...
    def test_logging_field_consistency(self):
        ...
        assert "provider" in optional_fields, "Provider should be optional field"
        assert "dimension" in optional_fields, "Dimension should be optional field"
    ...
    def test_event_level_values(self):
        ...
        with (
            ...
            chunks_file = self.temp_path / "test_chunks.ndjson"
            chunks_file.write_text(json.dumps({"chunk_id": "test:0001", "text_md": "Test content"}) + "\n")
            ...
            if output.strip():
                ...
                for event in events:
                    if "level" in event:
                        level = event["level"]
                        assert level in valid_levels, f"Invalid event level: {level}"
        ...
        with (
            ...
            if output.strip():
                ...
                for event in events:
                    if "stage" in event:
                        stage = event["stage"]
                        assert stage in valid_stages, f"Invalid stage name: {stage}"
