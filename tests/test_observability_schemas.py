...
def test_event_emitter_creates_ndjson():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        with EventEmitter(
            "test-run", "ingest", "confluence", log_dir=tmpdir
        ) as emitter:
        ...
        assert warning_event["metadata"]["message"] == "Test warning message"
...
def test_heartbeat_event_structure():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        with EventEmitter(
            "test-run", "embed", "openai", log_dir=tmpdir
        ) as emitter:
            emitter.heartbeat(
                processed=150,
                rate=2.5,
...
def test_stable_event_names():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        with EventEmitter("test", "test", "test", log_dir=tmpdir) as emitter:
