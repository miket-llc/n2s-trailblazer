...
# PROMPT DEV-022 — Retrieval on pgvector + ask (Deterministic, Observable)

Save as: prompts/022_retrieval_pgvector_and_ask.md
Branch: main
PASTE prompts/000_shared_guardrails.md VERBATIM ABOVE THIS PROMPT.

Context for a New Instance
Where we are: Confluence/DITA are ingested & normalized (var/runs/…); DEV-021 adds embeddings in Postgres+pgvector.

Hard rules: ingest/normalize remain DB-free; Postgres+pgvector only for retrieval; var/ workspace only; progress (stderr) + NDJSON logs (stdout); tests must pass.

Expected CLI: trailblazer db [check|init], trailblazer embed load … (from DEV-021), and now this prompt adds a small retriever and ask.

## To-Dos (≤ 9)

### Retriever core (pgvector)

DenseRetriever(db_url, provider="dummy"):

embed_query(q) → vector (same provider as embeddings).

search(q, top_k=8) → list of {chunk_id, doc_id, title, url, text_md, score} via pgvector \<=> cosine in SQL; Python fallback only in tests.

### Ranking & ties

Order by score DESC, ties by doc_id ASC, chunk_id ASC. No network rerankers in tests.

### Context packer

pack_context(hits, max_chars=6000); never split code blocks; include the first media placeholder per doc when present; return context_str + selected_hits.

### ask CLI

trailblazer ask "<question>" [--top-k 8] [--max-chunks-per-doc 3] [--provider dummy] [--max-chars 6000] [--format text|json] [--out DIR]

Writes hits.jsonl (one per line), summary.json (query, provider, timings, counts), context.txt; prints a brief top-N table when --format text.

### Observability

NDJSON events: search.begin, search.batch, search.end (stdout).

Heartbeat for very long searches; progress to stderr (human).

### Tests

Deterministic ranking with dummy vectors.

Packer respects boundaries and max_chars; includes a media placeholder when present.

CLI smoke writes all artifacts.

### Docs

README "Ask": examples, artifacts, note on media placeholders.

### Non-regression

No DB calls in ingest/normalize paths; db check must pass before retrieval paths.
...
Commit: feat(retrieval): pgvector dense retriever + context packer + ask CLI

**Acceptance:** ask is deterministic; artifacts present; tests green; ingest/normalize boundary respected.
