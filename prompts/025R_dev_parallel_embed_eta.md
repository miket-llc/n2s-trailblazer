...
1. Re-embed script: write a plan into progress (docs_planned/run)
1. Re-embed script: ensure docs_embedded always increments (even 0)
1. Dispatcher: set a safe default worker count and pass through env
1. Monitor: worker-aware ETA (use plan + live PIDs + EWMA rate)
1. Default WORKERS=2 in docs & Makefile
1. Tests & shellcheck (keep green)
1. Docs
1. Commit & push

Acceptance

- make reembed.openai.all runs with 2 workers by default.
- make embed.monitor displays docs embedded / planned, elapsed, EWMA doc rate, active_workers, and a correct ETA that converges as the run proceeds.
- No pagers triggered; Postgres-only DB usage; tests remain green.
