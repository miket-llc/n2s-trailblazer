...
from ..db.engine import Base, get_engine, get_session, get_db_url
...
def db_doctor(
    ...
    try:
        ...
        db_url = get_db_url()
        console.print(
            f"✅ Database URL configured: {db_url.split('@')[0]}@[REDACTED]"
        )
        ...
        console.print(
            f"[bold red]❌ Database health check failed: {e}[/bold red]"
        )
        raise typer.Exit(1)
...
def init_db(
    ...
    if drop_existing and not confirm:
        if not typer.confirm(
            "⚠️  This will DROP ALL EXISTING DATA. Are you sure?"
        ):
    ...
    try:
        ...
        if provider_stats:
            ...
            provider_table.add_column(
                "Embeddings", style="bold green", justify="right"
            )
        ...
        console.print(
            f"[bold red]❌ Failed to get database stats: {e}[/bold red]"
        )
        raise typer.Exit(1)
