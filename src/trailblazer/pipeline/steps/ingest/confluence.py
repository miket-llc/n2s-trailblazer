...
from ....core.event_log import get_event_logger, init_event_logger
from ....core.logging import log
from ....core.models import Attachment, ConfluenceUser, Page, PageAncestor
...
def _body_html_from_v2(page_obj: dict) -> str | None:
...
def _page_url(site_base: str, page_obj: dict) -> str | None:
...
def _map_page(
    ...
    space_key_unknown_count: dict[str, int] | None = None,
    space_details_cache: dict[str, dict] | None = None,
) -> Page:
    ...
    if space_key and space_details_cache is not None:
        if space_key not in space_details_cache and client:
            try:
                if hasattr(client, "get_space_details"):
                    space_details_cache[space_key] = client.get_space_details(space_key)
                else:
        ...
        if isinstance(space_details, dict):
            space_name = space_details.get("name") if isinstance(space_details.get("name"), str) else None
            space_type = space_details.get("type") if isinstance(space_details.get("type"), str) else None
    ...
    if client:
        ...
        try:
            if hasattr(client, "get_page_labels"):
                ...
                labels = [label.get("name", "") for label in label_data if label.get("name")]
        except Exception:
    ...
    page = Page(
        ...
        updated_at=(
            datetime.fromisoformat(version["createdAt"].replace("Z", "+00:00")) if version.get("createdAt") else None
        ),
...
def _resolve_space_key(
    ...
    """
    if not space_id:
        space_key_unknown_count["__missing_space_id__"] = space_key_unknown_count.get("__missing_space_id__", 0) + 1
        return "__unknown__"
    ...
    try:
        ...
        log.debug("space_key_api_lookup_failed", space_id=space_id, error=str(e))
    ...
    # Last resort
    space_key_unknown_count[space_id] = space_key_unknown_count.get(space_id, 0) + 1
    # Log warning once per space_id
    if space_key_unknown_count[space_id] == 1:
        log.warning("space_key_resolution_failed", space_id=space_id, url=page_url)
...
def ingest_confluence(
        ...
        try:
            with open(progress_json_path) as f:
                ...
                if last_page_id and last_timestamp:
                    progress_renderer.resume_indicator(last_page_id, last_timestamp)
        except Exception as e:
            log.warning(
    ...
    if auto_since and (space_keys or space_ids):
        ...
        first_space_key = (
            space_keys[0] if space_keys else next(iter(space_key_by_id.values())) if space_key_by_id else None
        )
        if first_space_key:
                    ...
                    with open(state_file) as f:
                        ...
                        if "last_highwater" in state_data:
                            effective_since = datetime.fromisoformat(
                                state_data["last_highwater"].replace("Z", "+00:00")
                            )
                            log.info(
    ...
    if effective_since:
        cql = _cql_for_since(space_keys or list(space_key_by_id.values()), effective_since)
        start, ids = 0, []
        while True:
    ...
    last_highwater: datetime | None = None
    space_key_unknown_count: dict[str, int] = {}  # Track failed space_key resolutions

    # Open CSV writers
    with (
        ndjson_path.open("w", encoding="utf-8") as out,
        pages_csv_path.open("w", newline="", encoding="utf-8") as pages_csv_file,
        attachments_csv_path.open("w", newline="", encoding="utf-8") as att_csv_file,
    ):
        ...
        def write_page_obj(p: Page, obj: dict):
            ...
            page_links = []
            if repr_ == "storage":
                page_links = extract_links_from_storage_with_classification(page_dict.get("body_storage"), site_base)
            elif repr_ == "adf":
                page_links = extract_links_from_adf_with_classification(page_dict.get("body_adf"), site_base)
            ...
            if page_media and p.attachments:
                ...
                page_media = resolve_attachment_ids(page_media, attachment_dicts)
            ...
            for ancestor in p.ancestors:
                edges_data.append({"type": "PARENT_OF", "src": ancestor.id, "dst": p.id})
            ...
            breadcrumbs_data.append({"page_id": p.id, "breadcrumbs": breadcrumb_items})

            # Write NDJSON
            out.write(json.dumps(page_dict, ensure_ascii=False, sort_keys=True) + "\n")
                ...
                attachments_data.append(
                    {
                        ...
                        "file_size": (str(att.file_size) if att.file_size else ""),
            ...
            if event_logger:
                ...
                page_json = json.dumps(page_dict, ensure_ascii=False, sort_keys=True)
            ...
            if p.attachments:
                filenames = [att.filename for att in p.attachments if att.filename]
                log.info(
        ...
        if candidate_ids is not None:
            ...
            first_space_key = (
                space_keys[0]
                if space_keys
                else (next(iter(space_key_by_id.values())) if space_key_by_id else "unknown")
            )
            ...
            for pid in candidate_ids:
                ...
                while attachment_retry_count <= max_attachment_retries:
                    try:
                        attachments_fetched = list(client.get_attachments_for_page(page.id))
                        break
                    except Exception as e:
            ...
            # full space scans
            if space_id_list:
                target_spaces: list[str | None] = list(space_id_list)
            else:
                target_spaces = [None]  # None => all pages

            for sid in target_spaces:
                # Get space key for logging
                space_key = space_key_by_id.get(sid, "unknown") if sid else "all_spaces"
    ...
    summary_data: dict[str, Any] = {
        ...
        "progress_checkpoints": (written_pages // progress_every if progress_every > 0 else 0),
        # Traceability stats
        ...
        "checkpoints_written": (written_pages // progress_every if progress_every > 0 else 0),
        "spaces": {},
    }
    ...
    try:
        ...
        progress_renderer.console.print()
        progress_renderer.console.print("[bold green]📋 Assurance Reports Generated:[/bold green]")
    ...
    progress_renderer.console.print(f"📄 [blue]Data file:[/blue] [cyan]{ndjson_path.name}[/cyan]")
