...
def ingest_confluence(
    ...
    log.info("ingest.confluence.start",
             outdir=outdir,
    ...
    if space_keys:
        ...
        for space in client.get_spaces(keys=space_keys):
            if space.get("key") in space_keys:
                ...
                log.info("ingest.confluence.resolved_space",
                        key=space.get("key"),
                        id=space["id"])
    
    if not resolved_space_ids:
        log.warning("ingest.confluence.no_spaces",
                   space_keys=space_keys,
                   space_ids=space_ids)
    ...
    if since:
        ...
        cql_parts = [
            "type=page",
            f'lastModified > "{since.isoformat()}"'
        ]
        ...
        for space_id in resolved_space_ids:
            log.info("ingest.confluence.fetching_pages",
                     space_id=space_id)
    ...
    spaces_processed = len([sid for sid in resolved_space_ids
                           if sid is not None])
            ...
            try:
                ...
                for att_data in client.get_attachments_for_page(page_id):
                    ...
                    if download_link.startswith("/"):
                        download_url = (client.base_url.rstrip("/wiki")
                                   + download_link)
                    else:
                ...
                page = Page(
                    ...
                    version=version_num,
                    body_html=(
                        page_data.get("body", {}).get("storage", {})
                        .get("value") or
                        page_data.get("body", {})
                        .get("atlas_doc_format", {}).get("value")
                    ),
                    url=page_url,
                ...
                if pages_processed % 10 == 0:
                    log.info("ingest.confluence.progress",
                             pages=pages_processed)
                    
            except Exception as e:
                log.error("ingest.confluence.page_error",
                         page_id=page_id,
                         error=str(e))
                continue
